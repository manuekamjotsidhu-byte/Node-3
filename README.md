...
- `/whitelist` - admin-only `add`, `remove`, or `list` command. Its `list` action fetches both the paid panel and FreeDash panel, shows every protected server with its panel label, labels paid servers as **Paid** and manually protected servers as **Whitelisted**, and automatically removes deleted servers from the whitelist. Purge protection checks server ID, UUID, and identifier.
