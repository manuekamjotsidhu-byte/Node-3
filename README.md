...
- `/purge confirm:True skip_keyword:smp` - admin-only purge for tracked free servers only; paid servers, whitelist matches by server ID/UUID/identifier, and names starting with the optional prefix such as `smp` or `[smp]` are not deleted.
