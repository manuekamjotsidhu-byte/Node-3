(() => {
  ...
  if (!window.ZeroXTheme) {
    ...
    const apply = raw => {
      ...
      if (!customStyle) { customStyle = document.createElement('style'); customStyle.id = 'zerox-theme-custom-css'; document.head.appendChild(customStyle); }
  ...
  const start = () => {
    ...
    const readForm = () => {
      ...
      ['reduceMotion', 'highContrast', 'hideFooter'].forEach(key => { value[key] = form.elements.namedItem(key).checked; });
    ...
    enabled.addEventListener('change', () => {
      ...
      writeForm(window.ZeroXTheme.save(settings));
